                ...
                for (int j = 0; j < seatsPerRow; j++) {
                    ...
                    if (seatIndex < seats.size()) {
                        ...
                        String seatHtml = String.format(
                            "<div class='%s' data-price='Price: ₹ %.2f' data-seat-id='%d' data-show-date='%s' data-show-time='%s' data-movie-id='%d' data-screen-id='%d' data-user-id='%s' title='%s' data-showtime-id='%d'>%d</div>",
                            seatClass, seatPrice, seat.getId(), showDate, showTime, movieId, screenId, userId, tooltipText, showtimeId, seat.getSeatNumber());
                        out.print(seatHtml);
                    }
                 ...
                 var options = {
                     ...
                     "handler": function (response) {
                         ...
                         fetch(confirmUrl, {
                             ...
                             .then(data => {
                                 console.log(data.id)
                                 window.location.href = `/admin/billBooking/` + data.id;
                             })
                             .catch(error => console.error('Error:', error));
                     },
                     "prefill": {
